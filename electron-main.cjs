...
function selectUserDataPath(){
  // Porto Prime 4.0.0: preserva automaticamente instalações ConstruGest existentes.
...
async function startServer(){const dataDir=path.join(app.getPath('userData'),'database');fs.mkdirSync(dataDir,{recursive:true});process.env.ELECTRON_DATA_DIR=dataDir;process.env.PORT='3210';process.env.HOST='0.0.0.0';process.env.JWT_SECRET=process.env.JWT_SECRET||crypto.randomBytes(32).toString('hex');await import('./src/server.js');}
